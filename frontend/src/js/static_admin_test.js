...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const userType = JSON.parse(user)?.type[0];
...
async function loadProducts() {
    ...
    try {
        ...
        if (result.status === "success") {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="btn btn-${
                            product.watchlist ? "danger" : "primary"
                        } wishlist-btn" data-product-id="${product.id}">
                            ${
                                product.watchlist ? "Remove from" : "Add to"
                            } Wishlist
                        </button>
                    </td>
                    <td>
            ...
            tbody.innerHTML =
                '<tr><td colspan="7" class="text-center">Error loading products: ' +
                result.message +
                "</td></tr>";
        }
    } catch (error) {
...
function populateFilterDropdowns() {
    ...
    if (retailerFilter) {
        ...
        const uniqueRetailers = [
            ...new Set(products.flatMap((product) => product.retailers)),
        ];
                ...
                btn.addEventListener("click", async function () {
                    ...
                    if (entity === "Product") {
                        ...
                        data = {
                            ...
                            [`${entity.toLowerCase()}_name`]: formData.get(
                                `${entity.toLowerCase()}Name`
                            ),
                            ...(entity === "Retailer" && {
                    ...
                    try {
                        ...
                        alert(
                            `${entity} ${
                                data.id ? "updated" : "saved"
                            } successfully! Message: ${result.message}`
                        );

                        const modal = bootstrap.Modal.getInstance(
                            document.getElementById(`add${entity}Modal`)
                        );
                        if (modal) {
                            modal.hide();
                        }
    ...
    document.addEventListener("click", async function (e) {
        ...
        if (e.target.closest(".edit-product")) {
            ...
            if (product) {
                ...
                form.querySelector('[name="productCategory"]').value =
                    categories.find((c) => c.name === product.category)?.id ||
                    "";
                form.querySelector('[name="productBrand"]').value =
                    brands.find((b) => b.name === product.brand)?.id || "";
            ...
            if (category) {
                ...
                form.querySelector('[name="categoryName"]').value =
                    category.name;
                const modal = new bootstrap.Modal(
            ...
            if (retailer) {
                ...
                form.querySelector('[name="retailerName"]').value =
                    retailer.name;
                form.querySelector('[name="retailerWebsite"]').value =
                    retailer.website;
                const modal = new bootstrap.Modal(
